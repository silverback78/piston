...
class Decks extends Response {
    ...
    public function Load() {
        ...
        DB::executeQuery('decks',"SELECT id, category, name, description FROM decks WHERE user_id = :userId", array(
            ':userId' => $this->userId
        ));
        ...
        DB::executeQuery('deckCategories',"SELECT DISTINCT category as name FROM decks WHERE user_id = :userId AND decks.category != '' ORDER BY decks.category ASC", array(
            ':userId' => $this->userId
        ));
